...
for i in range(1, 10000):
    randcolor = random.randint(0,0xffffffff)

    def draw(width, height, color=randcolor, backgroundColor=(0,0,0), perlinFactorW=2, perlinFactorH=2, step=0.001):
        ...
        for k, (x_s, y_s) in enumerate(POINTS):
            ...
            while c_len < MAX_LENGTH:
                ...
                sat = 190 * (MAX_LENGTH - c_len) / MAX_LENGTH
        ...
        save(p, fname=f'{i}', folder='pildid', overwrite=True)

    draw(1080, 1080, color=randcolor, perlinFactorW=2, perlinFactorH=2, step=0.0030)
